		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<table class="table table-striped" style="text-align: center; boder: 1px solid #ddd">
					<thead>
						<tr> 
							<th style="background-color: #eee; text-align: center;">리뷰 번호</th> 
							<th style="background-color: #eee; text-align: center;">리뷰 제목</th>
							<th style="background-color: #eee; text-align: center;">등록일</th>
							<th style="background-color: #eee; text-align: center;">적립금</th>
							<th style="background-color: #eee; text-align: center;">리뷰 확인</th>
							<th style="background-color: #eee; text-align: center;">리뷰 수정</th>
							<th style="background-color: #eee; text-align: center;">리뷰 삭제</th>
						</tr>
					</thead>
					<tbody>
					<c:if test="${totCnt > 0 }">
						<c:forEach var="board" items="${list3 }">
							<tr>
								<td>${board.posting_no}</td>
								<td>${board.board_title}</td>
								<td>${board.regi_date}</td>
								<td>${board.saving_pnt}</td>
								<td><a href="../board/boardContent.do?num=${board.posting_no}&pageNum=${currentPage}'">확인</a></td>
								<td><a href="../board/boardUpdateForm.do?posting_no=${board.posting_no}&pageNum=${currentPage}'">수정</a></td>
								<td><a onclick="return confirm('정말로 삭제하시겠습니까?')" href="MemberReviewListPro.do?posting_no=${board.posting_no }">삭제</a></td>
							</tr>
				...
				<table class="table table-striped" style="text-align: center; boder: 1px solid #ddd">
					<thead>
						<tr> 
							<th style="background-color: #eee; text-align: center;">숙소이름</th> 
							<th style="background-color: #eee; text-align: center;">후기 번호</th> 
							<th style="background-color: #eee; text-align: center;">후기 내용</th>
							<th style="background-color: #eee; text-align: center;">별점</th>
							<th style="background-color: #eee; text-align: center;">적립금</th>
							<th style="background-color: #eee; text-align: center;">후기 확인</th>						
							<th style="background-color: #eee; text-align: center;">후기 삭제</th>
						</tr>
					</thead>
					<tbody>			
					<c:if test="${totCntAccom > 0 }">
						<c:forEach var="accom_com" items="${list4}">
							<tr>
								<td>${accom_com.accom_name}</td>
								<td>${accom_com.com_no}</td>
								<td style="width:350px;">${accom_com.review_cont}</td>
								<td>${accom_com.accom_rat}</td>
								<td>${accom_com.saving_pnt}</td>
								<td><a href="../accomCom/accom_comList.do?accom_no=${accom_com.accom_no}&pageNum=${currentPage}">확인</a></td>								
								<td><a onclick="return confirm('정말로 삭제하시겠습니까?')" href="MemberReviewListPro2.do?accom_no=${accom_com.accom_no }&com_no=${accom_com.com_no}">삭제</a></td>
							</tr>
							<c:set var="startNum" value="${startNum - 1 }" />
						</c:forEach>
					</c:if>
			...
			<c:if test="${startPage > blockSize }">
				<a href='MemberReviewListForm.do?pageNum=${startPage-blockSize}'>[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href='MemberReviewListForm.do?pageNum=${i}'>[${i}]</a>
			</c:forEach>
			
			<c:if test="${endPage < pageCnt }">
				<a href='MemberReviewListFor.do?pageNum=${startPage+blockSize}'>[다음]</a>
			</c:if>
			</div>
										</div>
									</section>
