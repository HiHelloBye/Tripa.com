				...
				<!-- Main -->
					<div id="main">
						<div class="inner">
							...
							<!-- Header -->
								<header id="header">
									<a href="adminAccomWriteForm.jsp" class="logo"><strong>숙소 관리</strong> by ADMIN</a>
								</header>
							...
							<!-- Section -->
								<section>
									...
									<div class="content">
										<form action="adminAccomDeleteForm.do" method="post">
											<table>
											<caption><h4>기존의 숙소 정보를 삭제합니다</h4></caption>
												...
												<c:if test="${totCnt > 0 }">
													<c:forEach var="accom" items="${list }">
														<tr>
															<td><input type="checkbox" name="accom_no" value="${accom.accom_no}" id="${accom.accom_no}"><label for="${accom.accom_no}">${accom.accom_no}</label></td>
															<td class="left" width=200>${accom.accom_name}</td>
											...
											<div style="text-align: center;">
												<c:if test="${startPage > blockSize }">
													<a href='deleteList.do?pageNum=${startPage-blockSize}'>[이전]</a>
												</c:if>
												
												<c:forEach var="i" begin="${startPage}" end="${endPage}">
													<a href='deleteList.do?pageNum=${i}'>[${i}]</a>
												</c:forEach>
												
												<c:if test="${endPage < pageCnt }">
													<a href='deleteList.do?pageNum=${startPage+blockSize}'>[다음]</a>
												</c:if>
											</div>
											</div>
									</section>
