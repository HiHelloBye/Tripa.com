				...
				<!-- Main -->
					<div id="main">
						<div class="inner">
							...
							<!-- Header -->
								<header id="header">
									<a href="adminMemberList.do" class="logo"><strong>회원관리</strong> by ADMIN</a>
							...
							<!-- Section -->
								<section>
									...
									<div class="content">
										<form action="adminMemberDeleteForm.do" method="post">
											<table>
												<caption><h4>공지위반 회원을 삭제합니다</h4></caption>
													<tr style="background-color:#d5b6d6">
														<th>회원아이디</th><th>회원이름</th><th>회원이메일</th><th>회원전화번호</th><th>적립금</th>
													</tr>
													<c:if test="${totCnt > 0 }">
														<c:forEach var="member" items="${list }">
															<tr >
																<td><input type="checkbox" name="user_id" value="${member.user_id}" id="${member.user_id}"><label for="${member.user_id}">${member.user_id}</label></td>
																<td class="left" width=200>${member.user_name}</td>
																<td>${member.user_email}</td>
																<td>${member.user_cell}</td>
																<td>${member.saving_pnt}</td>
															</tr>
													...
													<c:if test="${totCnt == 0 }">
														<tr>
															<td colspan=7>회원 데이터가 존재하지 않습니다.</td>
														</tr>
													</c:if>
												</table>
												</form>
												<div style="text-align: center;">
													<c:if test="${startPage > blockSize }">
														<a href='adminMemberList.do?pageNum=${startPage-blockSize}'>[이전]</a>
													</c:if>
													
													<c:forEach var="i" begin="${startPage}" end="${endPage}">
														<a href='adminMemberList.do?pageNum=${i}'>[${i}]</a>
													</c:forEach>
													
													<c:if test="${endPage < pageCnt }">
														<a href='adminMemberList.do?pageNum=${startPage+blockSize}'>[다음]</a>
													</c:if>
												</div>
									</div>
								</section>
									...
									<ul>
										...
										<li>
											...
											<ul>
												...
												<li><a href="deleteList.do">숙소삭제</a></li>
											</ul>
										</li>
